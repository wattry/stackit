...
type RestackProgressCallback func(branchName string, result engine.RestackResult, newRev string, conflict bool, lockReason string, frozen bool)
...
func RestackBranchesWithHandler(ctx *app.Context, branches []engine.Branch, callback RestackProgressCallback) error {
	...
	if err != nil {
		if batchResult.ConflictBranch != "" {
			...
			if callback != nil {
				callback(batchResult.ConflictBranch, engine.RestackConflict, "", true, "", false)
			}
