...
Guide for resolving conflicts during `stackit absorb` operations. Unlike regular git conflicts, absorb conflicts occur when staged changes cannot be cleanly applied to their target commits in the stack.
...
stackit create "description of change"
```
...
Absorb failed midway. Run `stackit abort` to recover.
...
- `stackit log` shows clean structure
